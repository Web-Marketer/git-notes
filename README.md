...
9.1 **```git push --set-upstream origin HEAD```** - ```--set-upstream``` - синоним короткого ```-u```. HEAD — ссылка на последний коммит текущей ветки
...
- **```git merge --no-edit source```** - --no-edit избавляет от необходимости вводить сообщение для merge-коммит
